...
void
read_triangle_hierarchy_data(char *file,object_data *o){
    FILE *ifp;
    int i,j;
    int tmp[7];
    double ddum;
    int iprt=-1,icld[2]={-1,-1};
    double eb=0;
	char line[256];
	extern triangle_data BOUNDARY;
	extern vertex_data DUMMY_VERTEX;
	...
	fgets(line,256,ifp);
	sscanf(line,"%d%d%d",&o->num_of_ver,&o->num_of_init_tri,&o->num_of_all_tri);
    fprintf(stderr,"%d %d %d\n",o->num_of_ver,o->num_of_init_tri,o->num_of_all_tri);
    ...
    for (i=0; i<o->num_of_all_tri; i++){
		fgets(line,256,ifp);
        sscanf(line,"%d%d%d%d%d%d%d",tmp,tmp+1,tmp+2,&iprt,icld,icld+1,tmp+6);
        for (j=0; j<3; j++){
            o->triangles[i].ver[j] = &o->vertices[tmp[j]];
		}
        if (iprt == -1) o->triangles[i].parent = &BOUNDARY;
		...
		if(tmp[6] == -1) o->triangles[i].newvertex = &DUMMY_VERTEX;
		else o->triangles[i].newvertex = &o->vertices[tmp[6]];
	}
...
//	fprintf(ofp,"%d %d %d\n", wo->num_of_ver,wo->num_of_all_tri,0);
	...
	for (i=0; i<wo->num_of_ver; i++){
		fprintf(
			ofp,"%lf %lf %lf\n",
			wo->vertices[i].pos[0]/1000.0,
			wo->vertices[i].pos[1]/1000.0,
			wo->vertices[i].pos[2]/1000.0
		);
		wo->vertices[i].number = i;
	}
...
void
read_quad_data(char *file, object_data *o){
    FILE *fp;
    char line[256];
    int count_of_ver=0,count_of_tri=0,count_of_color=0;
    int i,j,itemp,flag;
...
//			printf("t1t%s",line);
	...
	o->num_of_all_tri = o->num_of_init_tri=count_of_tri;
	o->num_of_ver=count_of_ver;
//	o->number_of_cluster = o->num_of_init_tri;
	o->vertices = (vertex_data *)malloc(sizeof(vertex_data)*o->num_of_ver);
	o->triangles = (triangle_data *)malloc(sizeof(triangle_data)*o->num_of_init_tri);
	...
	while(fgets(line,256,fp) != NULL){
		...
		for(i=0;i<itemp;i++){
			...
			o->vertices[count_of_ver+i].pos[0] = temp_pos[0];
			o->vertices[count_of_ver+i].pos[1] = temp_pos[1];
			o->vertices[count_of_ver+i].pos[2] = temp_pos[2];
		}
		fgets(line,256,fp);
		//for(i=0;i<itemp;i++){
		//	o->vertices[count_of_ver+i].pos[0] = temp_pos[0];
		...
		//}
		for(i=0;i<itemp-2;i++){
			o->triangles[count_of_tri+i].ver[0] = &o->vertices[count_of_ver];
			for(j=1;j<3;j++){
				o->triangles[count_of_tri+i].ver[j] = &o->vertices[count_of_ver+(j+i)];
			}
		}
		count_of_ver+=itemp;
		count_of_tri+=itemp-2;
...
void
read_vch_data(char *file, object_data *o){
    FILE *fp;
	char line[1024],*string,*stopstring;
	int i,j,k,cnt1,cnt2,tmp[256];
	double dtmp;
	...
	fgets(line,1024,fp);
	sscanf(line,"%d%d",&o->num_of_ver,&o->number_of_cluster);
    fprintf(stderr,"%d %d\n",o->num_of_ver,o->number_of_cluster);
	...
	for(i=0;i<o->num_of_ver;i++){
...
#else
		sscanf(line,"%lf%lf%lf%lf%lf%lf%d%d%d%d",
#endif
            &o->vertices[i].pos[0],
            &o->vertices[i].pos[1],
            &o->vertices[i].pos[2],
		...
		o->vertices[i].bothcluster = (tmp[0] == -1) ? NULL : &o->clusters[tmp[0]];
		o->vertices[i].clusters[0] = (tmp[1] == -1) ? NULL : &o->clusters[tmp[1]];
		o->vertices[i].clusters[1] = (tmp[2] == -1) ? NULL : &o->clusters[tmp[2]];
		o->vertices[i].ring_number = tmp[3];
    }
	o->Nvpolygon = 0;
	for(i=0;i<o->number_of_cluster;i++){
		fgets(line,1024,fp);
		sscanf(line,"%d%d%d%d",&tmp[0],&tmp[1],&tmp[2],&tmp[3]);
		o->clusters[i].parent = (tmp[0] == -1) ? NULL : &o->clusters[tmp[0]];
		o->clusters[i].children[0] = (tmp[1] == -1) ? NULL : &o->clusters[tmp[1]];
		o->clusters[i].children[1] = (tmp[2] == -1) ? NULL : &o->clusters[tmp[2]];
	
		o->Nvpolygon += tmp[3];
		o->clusters[i].number_of_vertex = tmp[3];
	}
	printf("%d\n",o->Nvpolygon);
	...
	o->polygon_ver = (vertex_data **)malloc(sizeof(vertex_data *)*o->Nvpolygon+1);
	o->polygon_edge = (cluster_data **)malloc(sizeof(vertex_data *)*o->Nvpolygon+1);
	cnt1 = cnt2 = 0;
	...
	for(i=0;i<o->number_of_cluster;i++){
		...
		string = line;
		for(j=0;j<4;j++){
			strtol(string,&stopstring,0); string = stopstring;
		}
		o->clusters[i].polygon_ver = &o->polygon_ver[cnt1];
		for(j=0;j<o->clusters[i].number_of_vertex;j++){
			tmp[0] = strtol(string,&stopstring,0); string = stopstring;
			o->polygon_ver[cnt1++] = &o->vertices[tmp[0]];
		}
		o->clusters[i].polygon_edge = &o->polygon_edge[cnt2];
		for(j=0;j<o->clusters[i].number_of_vertex;j++){
			tmp[0] = strtol(string,&stopstring,0); string = stopstring;
//			o->polygon_edge[cnt2++] = &o->clusters[tmp[0]];
			o->polygon_edge[cnt2++] = (tmp[0] == -1) ? NULL : &o->clusters[tmp[0]];
		}
		dtmp = strtod(string,&stopstring); string = stopstring;
		o->clusters[i].direction_range = dtmp;
		for(j=0;j<3;j++){
			dtmp = strtod(string,&stopstring); string = stopstring;
			o->clusters[i].normal[j] = dtmp;
		}
		dtmp = strtod(string,&stopstring); string = stopstring;
		o->clusters[i].area = dtmp;
		dtmp = strtod(string,&stopstring); string = stopstring;
		o->clusters[i].quadrics_error = dtmp;
		dtmp = strtod(string,&stopstring); string = stopstring;
		o->clusters[i].correlation_error = dtmp;
		for(j=0;j<3;j++){
			for(k=0;k<3;k++){
				dtmp = strtod(string,&stopstring); string = stopstring;
//				o->clusters[i].p2n[j][k] = dtmp;
			}
		}
		for(j=0;j<3;j++){
				dtmp = strtod(string,&stopstring); string = stopstring;
				o->clusters[i].center[j] = dtmp;
		}
		for(j=0;j<3;j++){
				dtmp = strtod(string,&stopstring); string = stopstring;
				o->clusters[i].normal_center[j] = dtmp;
		}
	}
	printf("cnt %d %d %d \n",cnt1,cnt2,o->Nvpolygon);
	fclose(fp);
