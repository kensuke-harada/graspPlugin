...
namespace PathEngine {
  ...
  protected:
     ...
     */
		virtual int extend(RoadmapPtr tree, Configuration& qRand, bool reverse=false, CollisionChecker* col_checker = NULL);
     ...
     */
		int connect(RoadmapPtr tree, const Configuration& qNew, bool reverse=false, CollisionChecker* col_checker = NULL);
...
#ifdef ENABLE_RRT_MULTITHREAD
    bool is_succeed;
		int laststart_nid, lastgoal_nid;
		boost::mutex _mutex;
		boost::thread_specific_ptr<int> lastadded_start_nid_;
		boost::thread_specific_ptr<int> lastadded_goal_nid_;
#endif
  public:
    /**
     * @brief コンストラクタ
...
#ifdef ENABLE_RRT_MULTITHREAD
	protected:
		virtual bool extendParallel(grasp::RobotBodyPtr body, bool reverse);
#endif
  };
};
