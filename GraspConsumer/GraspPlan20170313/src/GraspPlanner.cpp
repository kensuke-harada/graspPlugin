...
static const char* graspplanner_spec[] =
  {
    ...
    // "conf.__type__.Position", "std::vector<double>",
    // "conf.__type__.Posture", "std::vector<double>",
...
GraspPlanner::GraspPlanner(RTC::Manager* manager)
  ...
  : RTC::DataFlowComponentBase(manager),
    m_PlanStartPortPort("PlanStartPort"),
...
RTC::ReturnCode_t GraspPlanner::onInitialize()
{
  ...
  m_PlanStartPortPort.registerProvider("PlanStart", "GraspPlanStart", m_PlanStart);
  
  // Set service consumers to Ports
  m_ResultPortPort.registerConsumer("Result", "GraspPlanResult", m_Result);
  
  // Set CORBA Service Ports
  addPort(m_PlanStartPortPort);
  ...
  bindParameter("Position", m_Position, "0,0,0");
